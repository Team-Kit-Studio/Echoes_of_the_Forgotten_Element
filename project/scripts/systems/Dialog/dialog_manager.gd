...
# Показать текст над любым персонажем (без ожидания завершения)
...
# То же, но с возвратом сигнала, чтобы катсцена могла дождаться конца
...
# Текст в координатах (ждёт пробел)
func show_text_at_position(text: String, position: Vector2, wait_for_input: bool = true) -> void:
	if current_text_box:
