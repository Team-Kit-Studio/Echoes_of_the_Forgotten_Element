...
func show_dialog(_npc, text = "", options = {}):
	if text != "":
		# Show empty box
...
func handle_dialog_choice(option):
	...
	if next_state == "end":
		if npc.current_branch_index < npc.dialog_resourse.get_npc_dialog(npc.npc_id).size() - 1:
		...
		if npc.dialog_resourse.get_npc_dialog(npc.npc_id)[npc.current_branch_index]["branch_id"] == "npc_default":
			offer_remaining_quests()
		else:
			offer_quests(npc.dialog_resourse.get_npc_dialog(npc.npc_id)[npc.current_branch_index]["branch_id"])
		show_dialog(npc)
	else:
		show_dialog(npc)
	
# в нужной ветке. патемся предложить все доступные на данный момент квесты
...
# в ветке по умолчанию, предлагаем все ранее непринятые квесты
