...
func _ready() -> void:
	...
	SaveSustem.create_new_saves.connect(Callable(self, "create_save_handler"))
...
func create_save_handler(_name: String) -> void:
	get_parent().get_owner().call_to_save()
	create_save_visual(_name)
...
func update_save_name_handler(save_node: Node) -> void:
	...
	SaveSustem.current_save = save_node
